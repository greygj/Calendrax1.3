...
import { ArrowLeft, Clock, MapPin, Star, Building2, Mail, ChevronRight, Image as ImageIcon, Shield } from 'lucide-react';
...
const PublicBusinessPage = () => {
  ...
  return (
    <div className="min-h-screen bg-appbg">
      {/* Header */}
      <header className="bg-cardBg border-b border-zinc-800 sticky top-0 z-10">
      ...
      <div className="h-64 bg-zinc-800 relative">
        {businessPhotos.length > 0 ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-cardBg">
            <img 
              src={business.logo} 
              alt={business.businessName}
          ...
          {services.length > 0 ? (
            <div className="space-y-3">
              {services.filter(s => s.active !== false).map(service => (
                <div 
                  key={service.id}
                  className="bg-cardBg border border-zinc-800 rounded-xl p-4"
                >
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-cardBg border-t border-zinc-800 p-4">
